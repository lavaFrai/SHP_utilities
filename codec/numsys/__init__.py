try:  # Python 2
    ...
    import codec.numsys.standard as _std
    import codec.numsys.nonstandard as _nsd
...
def in_decimal(num, sgn='-', sep='.', as_str=False):
    ...
    if as_str:
        ...
        split = split.replace(sgn, '');
        split = split.replace(sep, '')
...
def num_digits(base):
    ...
    if base == 0:
        return 0  # not actually a base
    elif 0 < base < 1:
        return int(_sup.ceil(one / base))
    elif base == 1:
        return 1  # same with this one
    elif base > 1:
        return int(_sup.ceil(base))
    else:
        raise E
