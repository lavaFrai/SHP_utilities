...
try:  # to look for better installed modules
    ...
    try:
        import codec.numsys.complex_decimal as cd
    except ImportError:
        import codec.numsys.complex_decimal as cd
    ...
    def mpc(a, b=None):
        ...
        if not b:
            return cd.ComplexDecimal(a)
        else:
            return cd.ComplexDecimal(a, b)
...
try:
    import codec.numsys.nonstandard as nstd  # used in parse_base
except ImportError:
    import codec.numsys.nonstandard as nstd
...
def unique(seq):
    ...
    seen = set();
    sead = seen.add  # http://stackoverflow.com/a/480227
...
def clean(num, sgn='-', sep='.'):
    ...
    if sgn and sgn in num:
        try:
            num = num.replace(sgn, '')  # str
        except:
            num = list(filter(lambda x: x != sgn, num))  # list
        neg = True
    else:
        neg = False
...
def str_to_lst(s, sgn='-', sep='.'):
    ...
    for char in s:
        if char == sgn:
            ...
            try:
                val = digitset.index(char)  # digit value
...
def parse_input(x):
    ...
    if name in str_types:
        ...
        try:
            real, imag = x.real, x.imag  # numeric types, named tuple
        except AttributeError:
            real, imag = x, 0  # anything else
    return real, imag
...
def parse_base(b):
    ...
    if name in str_types:
        b = str_(b).lower()
        if b in nstd.nstd_bases:
            pass
        elif 'i' in b or 'j' in b:
            b = mpc(b.replace('i', 'j'))
        else:
            try:
                b = int(b)
