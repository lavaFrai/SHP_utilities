...
def to_p1(n, **kwargs):
    ...
    sgn = kwargs.get('sgn', '-')
    if n < 0:
        return sgn + (_sup.digitset[1] * int(-n))
    elif n > 0:
        return _sup.digitset[1] * int(n)
    else:
        return ''
...
def to_n1(n, **kwargs):
    ...
    if n > 0:
        return _sup.digitset[1] * (int(n) * 2 - 1)
    elif n < 0:
        return _sup.digitset[1] * (-int(n) * 2)
    else:
        return ''
...
def _to_mixed(x, generator, **kwargs):
    ...
    whl = int(n)
    if whl != n:
        frc = _sup.mpf(n) - int(n)
    else:
        frc = 0  # if not enough precision, a "fraction" might appear
...
def _mixed_to(n, generator, **kwargs):
    ...
    for idx, (j, k) in enumerate(zip(whl[::-1], frc)):
        if j >= i:  # whole value error checking
            try:
                E = SyntaxError(_sup.str_(err1a).format(_sup.digitset[j], idx))
            except IndexError:
                E = SyntaxError(err1b.format(i))
            raise E
        if k >= i:  # fractional value error checking
            try:
                E = SyntaxError(_sup.str_(err2a).format(_sup.digitset[k], idx))
            except IndexError:
                E = SyntaxError(err2b.format(i))
            raise E
...
def _wpps():
    ...
    c = 11;
    gps = (2, 4, 2, 4, 6, 2, 6, 4, 2, 4, 6, 6, 2, 6, 4, 2, 6, 4, 6,
...
def _whatever(n=13, mode='stable'):  # almost like a damped spring
    ...
    n = abs(n);
    n = 1 if not n else n  # no infinite primes here!
...
def _simp(n=0):
    ...
    ni = 1;
    i = 1  # or are these figurate numbers of triangular type?
    while i < n: ni *= i; i += 1
    f = ni;
    nk = 1;
    k = 1
...
def _syl():
    ...
    s = 1;
    yield 2  # this grows too large too fast to be pratical as a base
